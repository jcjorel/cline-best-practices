...
This document details the implementation of the command structure for the `test llm bedrock` CLI command.
...
2. Create `src/dbp_cli/commands/test.py` with the `TestCommandHandler` class
3. Create `src/dbp_cli/commands/test/llm.py` with the `LLMTestCommandHandler` class
4. Create `src/dbp_cli/commands/test/bedrock.py` with the `BedrockTestCommandHandler` class
